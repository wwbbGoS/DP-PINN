...
class Schrodinger_PID():
    ...
    def n_phy_prob(self, X):
        ...
        nphy_u = torch.exp(- self.lambda_val * (f_u**2))
        nphy_v = torch.exp(- self.lambda_val * (f_v**2))
        return nphy_u, nphy_v, f_u, f_v, y_pred, noise
    ...
    def train_disriminator(self, x, y):
        ...
        real_prob_x0 = torch.ones(x.shape[0],2).to(self.device)
        real_logits_x0 = self.D.forward(torch.cat([x, y, real_prob_x0], dim=1))
        
        # physics loss for boundary points
        nphy_u_x0, nphy_v_x0, _, _, y_pred_x0, _ = self.n_phy_prob(x)
        fake_logits_x0 = self.D.forward(torch.cat([x, y_pred_x0, nphy_u_x0, nphy_v_x0], dim=1))
        
        # physics loss for collocation points
        nphy_u_xf, nphy_v_xf, _, _, y_pred_xf, _ = self.n_phy_prob(self.train_x_f)
        fake_logits_f = self.D.forward(torch.cat([self.train_x_f, y_pred_xf, nphy_u_xf, nphy_v_xf], dim=1))
        
        # computing synthetic real logits on collocation points for discriminator loss
        real_prob_f = torch.ones(self.train_x_f.shape[0],2).to(self.device)
        real_logits_f = self.D.forward(torch.cat([self.train_x_f, y_pred_xf, real_prob_f], dim=1))
        
        # discriminator loss
        d_loss = self.discriminator_loss(real_logits_x0, fake_logits_x0, fake_logits_f, real_logits_f)
    ...
    def train_generator(self, x, y):
        ...
        for gen_epoch in range(5):
            ...
            # physics loss for collocation points
            nphy_u_xf, nphy_v_xf, phyloss_u_xf, phyloss_v_xf, y_pred_xf, _ = self.n_phy_prob(self.train_x_f)
            fake_logits_f = self.D.forward(torch.cat([self.train_x_f, y_pred_xf, nphy_u_xf, nphy_v_xf], dim=1))

            # physics loss for boundary points
            nphy_u_x0, nphy_v_x0, phyloss_u_x0, phyloss_v_x0, y_pred_x0, G_noise = self.n_phy_prob(x)
            fake_logits_x0 = self.D.forward(torch.cat([x, y_pred_x0, nphy_u_x0, nphy_v_x0], dim=1))
            ...
            adv_loss = self.generator_loss(fake_logits_x0, fake_logits_f)
      
            b_loss = self.boundary_loss()

            # nphy_u_xf = nphy_u_xf.cpu().detach().numpy().flatten()
            # nphy_u_x0 = nphy_u_x0.cpu().detach().numpy().flatten()

            # aa = np.concatenate((nphy_u_xf, nphy_u_x0), axis=None)

            # g_loss = adv_loss  + self.lambda_q * mse_loss_Z + b_loss + (1 - (np.sum(aa)  / (aa.size)))

            g_loss = adv_loss  + self.lambda_q * mse_loss_Z + b_loss
    ...
    def train(self):
        ...
        return error_h_hist,error_f_hist